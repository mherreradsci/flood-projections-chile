...
- **Subcarpetas de `outputs/<region>/`:** todo archivo por-sufijo vive bajo `gfs/`, `ifs/` o `escenario/` (`utils.carpeta_fuente`/`ruta_salida`), nunca suelto en la raíz — el sufijo `gfs`/`ifs` mapea a su propia carpeta y cualquier otro sufijo (nombre de escenario, p. ej. `extremo_200mm`) cae en `escenario/`, compartida entre todos los escenarios sintéticos. `calibracion_reporte.csv` y `logs/` no son por-fuente y se quedan en la raíz de `outputs/<region>/`. `mapa.generar_mapa`, `flood_model.modelar_inundacion`, `new_areas.identificar_zonas_nuevas` y `exposure.evaluar_exposicion` usan `ruta_salida` (no `ruta_outputs`) para cualquier archivo etiquetado con `sufijo`.
...
- **Raster de susceptibilidad pareado con el mapa (`mapa.generar_mapa`):** además del HTML timestampeado, cada render copia `extension_<sufijo>.tif` (el working file que `flood_model`/`new_areas`/`exposure` leen con nombre fijo dentro de la misma corrida) a un archivo con el mismo nombre base que el HTML — "_extension" insertado después del sufijo, `.tif` en vez de `.html` — p. ej. `mapa_anegamientos_gfs_20260729_18utc_20260729-190059.html` ↔ `mapa_anegamientos_gfs_extension_20260729_18utc_20260729-190059.tif`. Existe para que un proceso externo que ya conoce el nombre del HTML derive el del raster con una regla fija (insertar `_extension` después del sufijo, cambiar la extensión), sin ambigüedad de "cuál corrida generó este HTML" cuando `outputs/` acumula varios renders del mismo ciclo. Aplica a cualquier sufijo, con o sin ciclo (escenarios sintéticos incluidos); `limpieza._raster_de_mapa` deriva el mismo nombre, pero solo se ejercita sobre HTML que matchean `PATRON_MAPA` (gfs/ifs con ciclo) — los renders de escenario quedan fuera de la poda de duplicados igual que su HTML.
- **Poda de `outputs/` (`limpieza.py` + `scripts/05_limpiar_outputs.py`):** el historial crece sin techo (~4 MB por HTML, más su raster pareado) y además acumula *duplicados por ciclo*, porque el nombre lleva ciclo **y** timestamp de render: volver a proyectar un ciclo agrega un archivo en vez de reemplazarlo. `_por_ciclo` usa `rglob`, no `glob`, para recorrer las subcarpetas `gfs/ifs/escenario`. `reprocesar_ciclo_gfs.py` evita crearlos avisando, pero la ruta del cron (`04_proyectar.py`) no. La poda de duplicados no pierde información (los renders viejos del mismo ciclo son intentos superados); `--conservar-ciclos N` sí descarta historial, por eso es opt-in. Todo simula por default: `--aplicar` es obligatorio para borrar, ya que `outputs/` está en `.gitignore` y no se recupera. `limpieza.PATRON_MAPA` es la dirección inversa de `utils.ciclo_tag` — si se cambia el formato del tag hay que tocar ambos, y `tests/test_limpieza.py` los ata componiendo uno con otro. `limpieza._raster_de_mapa` deriva el nombre del raster pareado a partir de un match de `PATRON_MAPA`, así que un HTML sobrante siempre se poda junto con su raster (si existe; corridas viejas anteriores a este cambio no tienen raster pareado y no rompen la poda).
...
- **Cron de corridas recurrentes (`correr_proyeccion_{,atacama_,antofagasta_}{gfs,ifs}.sh`, documentado con detalle en el propio `crontab -l`):** corre en la zona horaria local del sistema (`America/Santiago`, verificar con `timedatectl` — no asumir UTC) mientras los ciclos GFS/IFS son UTC (00/06/12/18), así que hora local `H` en el cron cubre UTC `H+4` (o el offset vigente). El open-data IFS 0.25° tarda ~6.5 h en publicarse tras la hora nominal del ciclo (medido 2026-07-30: ciclo 06z listo a las 12:27 UTC), más que GFS (~4 h). Aun así, la corrida recurrente (`04_proyectar.py --fuente {gfs,ifs}`, sin `--ciclo`) nunca falla por esto: "el ciclo más reciente" se resuelve solo (heurística propia `sondear_ciclos_gfs` para GFS, `ecmwf.opendata Client.latest()` para IFS) y cae al ciclo anterior sin error si el más nuevo no está listo — el horario del cron solo decide el margen de frescura, no la estabilidad. Distinto es el backfill puntual (`reprocesar_ciclo_{gfs,ifs}.py --ciclo <fecha>`), que fuerza un ciclo exacto sin ese fallback: puede `HTTPError 404` si se pide un ciclo del día antes de que se publique, o si el ciclo es futuro (este último cortado antes de tocar red por `utils.validar_ciclo_no_futuro`).
...
- `sentinel1.filtrar_oscuro_permanente` descarta celdas oscuras en todos los eventos S1 (salares y arenales lisos al radar). Sin él, en Atacama la mitad del "agua" es salar y los factores de calibración saturan en factor_max. Activo en Atacama y Antofagasta (donde el Salar de Atacama domina la señal oscura; ojo: el filtro necesita ≥2 eventos S1 con dato, con uno solo no filtra nada), apagado en Coquimbo (huellas ya calibradas).
