#!/usr/bin/env bash
# Wrapper para cron/systemd: corre 04_proyectar.py --fuente ifs --config config_antofagasta.yaml con log por corrida.
...
exec 9>"$LOGDIR/.proyeccion-antofagasta-ifs.lock"
...
"$REPO/.venv/bin/python" scripts/04_proyectar.py --fuente ifs --config config_antofagasta.yaml
