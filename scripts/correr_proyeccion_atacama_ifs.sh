#!/usr/bin/env bash
# Wrapper para cron/systemd: corre 04_proyectar.py --fuente ifs --config config_atacama.yaml con log por corrida.
...
exec 9>"$LOGDIR/.proyeccion-atacama-ifs.lock"
...
"$REPO/.venv/bin/python" scripts/04_proyectar.py --fuente ifs --config config_atacama.yaml
